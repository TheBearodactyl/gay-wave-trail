...
bool ColorListSettingNode::init(std::shared_ptr<ColorListSetting> setting, float width) {
    ...
    auto view_btn = CCMenuItemSpriteExtra::create(
            view_spr,
            this,
            menu_selector(ColorListSettingNode::onView)
    );
...
SettingNodeV3 *ColorListSetting::createNode(float width) {
    return ColorListSettingNode::create(std::static_pointer_cast<ColorListSetting>(
            shared_from_this()
    ), width);
}
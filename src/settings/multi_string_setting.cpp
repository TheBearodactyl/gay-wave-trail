...
protected:
  ...
  void createList() {
    ...
    for (int i = 0; i < m_localValue.size(); i++) {
      ...
      auto inputNode = TextInput::create(103.f, "Color Hex", "chatFont.fnt");
      inputNode->setScale(1.f);
      inputNode->setPosition(size2.width / 2 - 15.f, size2.height / 2);
      inputNode->setFilter("#0123456789abcdefABCDEF");
...
bool MultiStringSettingNode::init(MultiStringSettingValue *value, float width) {
  ...
  auto settingJson = Mod::get()->getSettingDefinition("colors-list")->get<CustomSetting>()->json;

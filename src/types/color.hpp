...
struct ColorEntry final {
  public:
	...
	bool operator==(const ColorEntry& other) const {
		return m_hex == other.m_hex and m_userAdded == other.m_userAdded and m_enabled == other.m_enabled;
	}

	ColorEntry(std::string m_hex, bool m_userAdded, bool m_enabled) :
		m_hex(std::move(m_hex)),
		m_enabled(m_enabled),
		m_userAdded(m_userAdded) {}

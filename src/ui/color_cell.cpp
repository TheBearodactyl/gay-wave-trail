...
bool ColorCell::init(ColorListPopup *parent_popup, ColorEntry *entry, std::function<void()> on_delete, const cocos2d::CCSize &size) {
  if (! CCNode::init())
    return false;
  ...
  input_node->setCallback([this](std::string const &str) {
    m_entry->m_hex = str;
  });
...
ColorCell *ColorCell::create(ColorListPopup *parent_popup, ColorEntry *entry, std::function<void()> on_delete, const CCSize &size) {

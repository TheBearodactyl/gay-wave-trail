...
bool GwtSettingsPopup::setup() {
	...
	let full_bg = CCLayerColor::create(rp_c4(rp::surface, 255));
	let full_bg_size = m_mainLayer->getContentSize() - CCSize {6.f, 6.f};
	...
	let popup_border = CCLayerColor::create(
		rp_c4(rp::h_med, 255),
		full_bg_size.width + gay::OUTER_BORDER * 2.f,
		full_bg_size.height + gay::OUTER_BORDER * 2.f
	);
	...
	let title_label = CCLabelBMFont::create("Settings for Gay Wave Trail", "goldFont.fnt");
	...
	CCSize list_bg_size = {LIST_W + gay::PAD * 2, LIST_H + SEARCH_H + gay::PAD * 2};
	...
	let list_border = CCLayerColor::create(
		rp_c4(rp::overlay, 220),
		list_bg_size.width + gay::LIST_BORDER * 2.f,
		list_bg_size.height + gay::LIST_BORDER * 2.f
	);
	...
	let list_inner = CCLayerColor::create(rp_c4(rp::surface, 180));
	...
	m_search->setCallback([this](const std::string& q) { on_search_changed(q); });
	m_search->setID("search-input");
	search_menu->addChildAtPosition(m_search, Anchor::Left, ccp(16.f, 0.f), ccp(0.f, 0.5f));

	let clr_spr = CCSprite::createWithSpriteFrameName("GJ_deleteIcon_001.png");
	...
	let search_separator = CCLayerColor::create(rp_c4(rp::overlay, 180), LIST_W, 1.5f);
	...
	let close_spr = ButtonSprite::create("Close", "goldFont.fnt", "geode.loader/GE_button_05-uhd.png", 0.7f);
	...
	let btn_half_gap = m_apply_btn->getScaledContentWidth() / 2.f + 6.f;
	...
	for (let ch : m_mainLayer->getChildrenExt()) {
...
void GwtSettingsPopup::on_search_changed(const std::string& query) {
	...
	let active = !query.empty();
	let spr = static_cast<CCSprite*>(m_clear_btn->getNormalImage());
...
void GwtSettingsPopup::rebuild_list(bool preserve_scroll) {
	...
	let query = m_search ? std::string(m_search->getString()) : std::string {};
	let has_query = !query.empty();
	...
	Vec<ScoredInfo> scored;

	for (const let& info : SETTING_DISPLAY_INFO) {
		...
		let name = std::string(mt.name);
		let key = std::string(info.key);
		let nr = gay::util::fuzzy_match(query, name);
		let kr = gay::util::fuzzy_match(query, key);
	...
	if (!m_list) {
		...
		m_list->m_contentLayer->setLayout(
			ColumnLayout::create()
				...
				->setAutoGrowAxis(LIST_H)
				->setGap(0.f)
		);

		let inner = m_list_bg->getChildByID("list-inner-layer-color");

		if (inner) {
			inner->addChildAtPosition(m_list, Anchor::BottomLeft);
		} else {
			static_cast<CCNode*>(m_list_bg)->addChildAtPosition(m_list, Anchor::BottomLeft);
		}
	}
	...
	for (const let& s : scored) {
		const let& info = *s.info;

		if (!has_query && !info.section_key.empty() && info.section_key != cur_section) {
			cur_section = info.section_key;

			for (const let& sec : SECTION_INFO) {
				if (sec.key != info.section_key) {
					continue;
				}

				let title = gay::get_section_title(sec, static_cast<int>(mode));

				if (let hdr = GwtSectionHeader::create(title, sec.tooltip, LIST_W)) {
		...
		let row = GwtSettingRow::create(info, mode, LIST_W, this);
	...
	let saved_y = m_list->m_contentLayer->getPositionY();
...
void GwtSettingsPopup::on_apply(CCObject*) {
	for (const let& [key, val] : m_pending) {
		for (const let& info : SETTING_DISPLAY_INFO) {
			if (info.key != key) {
				continue;
			}
			...
			switch (info.type) {
				...
				case ST::ColorList: {
					let r = matjson::Serialize<gay::ColorList>::fromJson(val);
					if (r.isOk()) {
...
void GwtSettingsPopup::update_apply_btn() {
	...
	let dim = [](CCMenuItemSpriteExtra* btn, bool en) {
		if (!btn) {
			return;
		}
		...
		if (let spr = static_cast<CCSprite*>(btn->getNormalImage())) {

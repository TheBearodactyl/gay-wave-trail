...
class $modify(PlayLayer) {
  void postUpdate(float p0) override {
    ...
    if (this->m_player1 && gay::settings::get_mod_enabled() && gay::settings::get_rainbow_enabled() && this->m_player1->m_isDart) {
      this->m_player1->m_waveTrail->setColor(gay::settings::get_gaydient_enabled() ? gradient_one : rainbow_one);
    }

    if (this->m_player2 && gay::settings::get_mod_enabled() && gay::settings::get_rainbow_enabled() && this->m_player2->m_isDart) {
      auto player_two_color = gay::settings::get_mirror_players() ? (gay::settings::get_gaydient_enabled() ? gradient_one : rainbow_one) : (gay::settings::get_gaydient_enabled() ? gradient_two : rainbow_two);
      this->m_player2->m_waveTrail->setColor(player_two_color);
    }
  }
};
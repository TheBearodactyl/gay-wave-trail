...
struct TrailOutline : Modify<TrailOutline, cocos2d::CCDrawNode> {
    bool drawPolygon(CCPoint *verts, unsigned int count, const ccColor4F &fillColor, float borderWidth,
                     const ccColor4F &borderColor) {
        if ((fillColor.r == 1.F && fillColor.g == 1.F && fillColor.b == 1.F && fillColor.a != 1.F) ||
            ((s_currentStreak != this) && (s_currentStreak2 != this))) {
        ...
        if (settings::get<bool>("wave-outline")) {
            ...
            if (color_phase >= 360.0f) color_phase -= 360.0f;
            ...
            if (blur_layers == 0.0) {
                ...
                this->drawSegment(new_verts[0], new_verts[3], static_cast<float>(trail_outline_width),
                                  trail_outline_color);
                this->drawSegment(new_verts[1], new_verts[2], static_cast<float>(trail_outline_width),
                                  trail_outline_color);

...
class $modify(HardStreak) {
  void updateStroke(float p0) {
    if (LevelEditorLayer::get() && gay::settings::get_mod_enabled() && gay::settings::get_editor_trail()) {
      m_drawStreak = true;
    }
...
class $modify(PlayerObject) {
  void placeStreakPoint() {
    if (LevelEditorLayer::get() && this->m_isDart && gay::settings::get_mod_enabled() && gay::settings::get_editor_trail()) {

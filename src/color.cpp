...
namespace gay::color {
	...
	GLubyte lerp_channel(GLubyte a, GLubyte b, float t) {
		auto result = static_cast<GLubyte>(
			static_cast<int32_t>(a) + static_cast<int32_t>(static_cast<int32_t>(b) - static_cast<int32_t>(a)) * t
		);
		gay::util::log(
			LogLevel::Debug,
	...
	ccColor3B hex_to_rgb(std::string_view hex_code) {
		...
		int32_t value = geode::utils::numFromString<int>(std::string(clean), 16).unwrapOr(0);
	...
	RustStr rgb_to_hex(const ccColor3B& color) {
	...
	RustStr rgb_to_hex(const ccColor4B& color) {
		gay::util::log(
			LogLevel::Debug,
			"[rgb_to_hex (4B)] rgba=({},{},{},{})",
			(int)color.r,
	...
	ccColor3B get_rainbow(float offset, float saturation) {
		...
		int32_t sector_idx = static_cast<int32_t>(sector_pos);
	...
	ccColor3B get_gradient(float phase, float offset, bool smooth, const ColorList& colors) {
		...
		auto clrs = colors.colors | std::views::filter([](const ColorEntry& e) { return e.enabled; }) |
		...
		int32_t count = static_cast<int32_t>(clrs.size());
		float segment_size = 360.0f / static_cast<float>(count);
		float segment_float = normalized / segment_size;
		int32_t current = static_cast<int32_t>(segment_float) % count;
		float t =
			std::clamp<float>(segment_float - static_cast<float>(static_cast<int32_t>(segment_float)), 0.0f, 1.0f);
	...
	ccColor3B apply_brightness(const ccColor3B& color, float brightness) {
		...
		auto result = ccColor3B {static_cast<GLubyte>(std::min<float>(255.0f, color.r * brightness)),
			static_cast<GLubyte>(std::min<float>(255.0f, color.g * brightness)),
			static_cast<GLubyte>(std::min<float>(255.0f, color.b * brightness))};
		gay::util::log(
			LogLevel::Debug,
